...
If you forked the full repository, use [`examples/workflows/htb-metrics-fork.yml`](../../examples/workflows/htb-metrics-fork.yml) instead. It runs `htbctrl badges generate` from the checked-out fork (no second checkout).
...
2. `pip install -r requirements.txt` (editable install of `src/ctrl_metrics`)
3. `playwright install chromium --with-deps`
4. `htbctrl badges generate -p $HTB_PROFILE_ID -t $TEMPLATE`
