...
HTML template rendering failed. Ensure the template has a root `.badge` element and vendor icons exist under `assets/vendor/icons/`.
...
```bash
htbctrl badges generate -t nonexistent   # lists valid names in error
```
